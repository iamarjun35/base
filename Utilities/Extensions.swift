...
extension UIButton{
    ...
    func setAlamofireImage(string : String?){
        if let urlStr = string{
            if let url = URL(string: urlStr){
                self.af.setImage(for: .normal, url: url)
            }
        }
    }
...
//        NVActivityIndicatorPresenter.sharedInstance.startAnimating(activityData, NVActivityIndicatorView.DEFAULT_FADE_IN_ANIMATION)
        ...
        let activityData = ActivityData(size: size, message: "", messageFont: nil, messageSpacing: nil, type: .ballScaleRippleMultiple, color: nil, padding: nil, displayTimeThreshold: nil, minimumDisplayTime: nil, backgroundColor: bgColor, textColor: .white)
        NVActivityIndicatorPresenter.sharedInstance.startAnimating(activityData, NVActivityIndicatorView.DEFAULT_FADE_IN_ANIMATION)
    }
    func hideHudd(){
...
//        SVProgressHUD.setDefaultMaskType(.none)
    ...
    func sendTokenMultipart() -> HTTPHeaders{
    ...
    func getAttributedString(boldText:String,italicText:String) -> NSMutableAttributedString{
        ...
        let attributedString = NSMutableAttributedString(string: italicText, attributes:attributsNormal)
...
extension String {
    ...
    func addCurrency()->String{
        return "\(self) \(KCurrencySymbol)"
    }
    
    func strikeThrough()->NSMutableAttributedString?{
//        let attributeString: NSMutableAttributedString =  NSMutableAttributedString(string: self)
        let attributeString: NSMutableAttributedString?
        if self.isCompletelyEmpty() == true{
            attributeString = NSMutableAttributedString(string: self)
        }else{
            attributeString = NSMutableAttributedString(string: self.addCurrency())
...
extension UserDefaults {
    func incrementIntegerForKey(key:String, by: Int) {
        if AppUserDefaults.integer(forKey: "swipedCards") == 10{
            set(1, forKey:key)
        }else{
